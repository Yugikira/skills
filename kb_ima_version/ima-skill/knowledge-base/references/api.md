...
POST /openapi/wiki/v1/move_knowledge

**触发场景**：将文件从一个知识库移动到另一个知识库，支持批量移动（最多10个文件）。

#### 请求参数

| 字段                    | 类型     | 必填 | 说明                                    |
| ----------------------- | -------- | ---- | --------------------------------------- |
| `src_knowledge_base_id` | string   | 是   | 源知识库 ID                             |
| `dst_knowledge_base_id` | string   | 是   | 目标知识库 ID                           |
| `dst_folder_id`         | string   | 否   | 目标文件夹 ID（省略则移到根目录）       |
| `src_knowledge_base_name` | string | 否 | 源知识库名称，用于二次校验             |
| `dst_knowledge_base_name` | string | 否 | 目标知识库名称，用于二次校验           |
| `dst_folder_name`       | string   | 否   | 目标文件夹名称，用于二次校验           |
| `infos`                 | array    | 是   | 要移动的文件列表（最多10个）            |

**infos 数组元素：**

| 字段       | 类型   | 必填 | 说明                  |
| ---------- | ------ | ---- | --------------------- |
| `media_id` | string | 是   | 要移动的文件/文件夹 ID |

#### 返回字段

| 字段          | 类型                       | 说明               |
| ------------- | -------------------------- | ------------------ |
| `move_results` | map\<string, MoveResult\> | media_id → 结果映射 |

**MoveResult：**

| 字段       | 类型   | 说明          |
| ---------- | ------ | ------------- |
| `ret_code` | int32  | 0=成功，非0=失败 |
| `err_msg`  | string | 错误信息      |
...
curl -X POST "https://ima.qq.com/openapi/wiki/v1/move_knowledge" \
    ...
    -d '{
      "src_knowledge_base_id": "concepts_kb_id",
      "dst_knowledge_base_id": "archived_kb_id",
      "infos": [
        { "media_id": "old_concept_1" },
        { "media_id": "old_concept_2" }
      ]
    }'
```
...
- `infos` 数组最多放 10 个 `media_id`
- 源和目标可以是不同知识库
- `dst_folder_id` 不传则移到根目录
- 传 `*_name` 字段会额外做名称匹配验证，防止传错 ID
- 获取 `media_id`：通过 `get_knowledge_list` 或 `search_knowledge` 接口
- 获取 `kb_id`：通过 `search_knowledge_base` 接口
